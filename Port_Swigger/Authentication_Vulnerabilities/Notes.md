...
A secure system should respond in such a way that does not reveal which of those two checks failed.</br>
...
Even if message is same → timing leaks existence.

## How to detect (manual testing)

Write checklist:
- Compare valid vs invalid usernames
- Observe:
    - message
    - timing
    - status code
...
- Brute force → needs valid usernames first
- Credential stuffing → uses enumerated usernames
- Password spraying → uses known usernames
- Authentication bypass → sometimes exposed via enumeration flows